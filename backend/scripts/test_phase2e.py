"""Phase 2E integration verification."""
    ...
    try:
        ...
        for index in range(3):
            participant = Participant(
                public_id=f"MLPRED{index}{uuid4().hex[:6].upper()}",
...
def main():
    from app.services.ml_inference import risk_level
    ...
    print("risk levels OK")

    token = researcher_token()
    seed_training_cohort()

    _, ds = req("POST", "/v1/research/datasets/build", {"name": "phase-2e-set"}, token=token)
    dataset_id = ds["id"]
    ...
    print("train OK", model_id)

    db = SessionLocal()
    participant = db.execute(
        select(Participant).where(Participant.public_id.like("MLPRED%")).order_by(Participant.created_at.desc())
    ).scalars().first()
    session = db.execute(
        select(DailySession)
    ...
    status, predict_resp = req(
        "POST",
        f"/v1/research/models/{model_id}/predict",
        {"participant_id": str(participant.id), "session_date": session.session_date.isoformat()},
        token=token,
    )
    assert status == 201, predict_resp
    assert 0.0 <= predict_resp["probability"] <= 1.0
    assert predict_resp["risk_level"] in {"low", "medium", "high"}
    assert 0.0 <= predict_resp["confidence"] <= 1.0
    assert "burnout_next_day" not in str(predict_resp)
    print("single predict OK", predict_resp["probability"], predict_resp["risk_level"])

    _, batch_resp = req("POST", f"/v1/research/models/{model_id}/batch-predict", token=token)
    assert batch_resp["successful_predictions"] >= 1
    print("batch predict OK", batch_resp)

    _, predictions = req("GET", "/v1/research/predictions", token=token)
    assert len(predictions) >= 1
    print("predictions list OK", len(predictions))

    _, history = req("GET", f"/v1/research/predictions/{participant.id}", token=token)
    assert len(history) >= 1
    print("participant history OK", len(history))

    db = SessionLocal()
    count = db.execute(text("select count(*) from ml_predictions")).scalar_one()
    db.close()
    assert count >= 1
    print("db predictions saved OK", count)

    preg = json.loads(
        urllib.request.urlopen(
            urllib.request.Request(
                BASE + "/v1/auth/participant/register",
                data=json.dumps({"grade": "9th Grade", "age_range": "13-14", "pet_choice": "fox", "pin": "9393"}).encode(),
                headers={"Content-Type": "application/json"},
                method="POST",
            )
        ).read().decode()
    ...
    for path, method, body in [
        (f"/v1/research/models/{model_id}/predict", "POST", {"participant_id": str(participant.id), "session_date": session.session_date.isoformat()}),
        (f"/v1/research/models/{model_id}/batch-predict", "POST", None),
        ("/v1/research/predictions", "GET", None),
    ]:
        code, _ = req(method, path, body, token=preg["access_token"], expect_error=True)
